  ...
  const updateCartCount = (item, change) => {
    setCartItems((prevItems) => {
      ...
      if (existingItemIndex > -1) 
      {
        ...
        const updatedQuantity = updatedItems[existingItemIndex].quantity + change;
        if (updatedQuantity <= 0) {
          updatedItems.splice(existingItemIndex, 1); // Remove the item if quantity <= 0
        } 
        else 
        {
          updatedItems[existingItemIndex].quantity = updatedQuantity; // Update quantity
        }
          return updatedItems;
        }
        else if (change > 0) 
